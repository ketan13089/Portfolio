...
export default function Navbar() {
    ...
    const navVariants = {
        ...
        return (
            ...
            >
                <div className="w-screen bg-gradient-to-r from-blue-200 to-indigo-300 rounded-b-xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex justify-between h-16 items-center">
                        <motion.div className="text-2xl font-bold" whileHover={{ scale: 1.05 }}>
                            <a href="/about">
                                ...
                                <span className="text-2xl text-red-600 font-bold transition-all duration-300 ease-in-out hover:text-red-800 px-0.25">
                                    G
                                </span>
                            </a>
                        </motion.div>
                        ...
                        <div className="flex justify:center space-x-8">
                            ...
                            {navItems.map((item) => (
                                <motion.div key={item.name} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
                                    {item.path ? (
                                        <Link to={item.path} className="text-gray-700 hover:text-indigo-600 transition-colors">
                                            {item.name}
                                        </Link>
                                    ) : (
